                     ...
                     if ($(dfx_str_fwbh).text() != oldzhi_fwbh) {
                         ...
                         if (arr != null) {//如果能匹配成功即arr数组不为空，循环输出结果
                             for (var i = 0; i < arr.length; i++) {
                                 ...
                                 if (arr_z[0] == "[季度租金")
                                 { $("#Hjdzj").val($.trim(arr_z[1]).replace("]", "")); }
                                 if (arr_z[0] == "[季度物业费")
                                 { $("#Hjdwyf").val($.trim(arr_z[1]).replace("]", "")); }
                                 if (arr_z[0] == "[季度电梯费")
                                 { $("#Hjddtf").val($.trim(arr_z[1]).replace("]", "")); }
                                 if (arr_z[0] == "[季度其他费")
                                 { $("#Hjdqtfy").val($.trim(arr_z[1]).replace("]", "")); }
                             }
                         }
            ...
            function add_anniu_spsp()
            {
                ...
                if ($("#fifsssss_Hzhuangtai").text() == "草稿") {
                    ...
                    $(document).on('click', "#" + bjm + "_top", function () {
                        ...
                        begin_ajax("shengxiao", getUrlParam("idforedit"), "160622000081")
