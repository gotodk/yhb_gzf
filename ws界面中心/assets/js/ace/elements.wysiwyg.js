...
(function($ , undefined) {
	$.fn.ace_wysiwyg = function($options , undefined) {
		...
		var color_values = [
			'#ffffff','#d06b64','#f83a22','#fa573c','#ff7537','#ffad46',
		...
		{
			'font' : {
			    values: ['Arial', 'Tahoma', '宋体', '微软雅黑', '黑体'],
				icon:'fa fa-font',
				title:'字体'
			},
			'fontSize' : {
			...
			'insertImage' : {
				...
				title : '插入图片',
				button_text : '<i class="'+ ace.vars['icon'] + 'fa fa-file"></i> 选择本地图片 &hellip;',
				placeholder : '图片网址...',
				button_insert : '插入',
				button_class : 'btn-success',
				button_insert_class : 'btn-primary',
				choose_file: false //show the choose file button?
			},
			'foreColor' : {
				values : color_values,
				title : '选择前景颜色'
		...
		this.each(function() {
		    var toolbar = ' <div class="wysiwyg-toolbar btn-toolbar center wysiwyg-style2"> <div class="btn-group"> ';
			...
			for(var tb in toolbar_buttons) if(toolbar_buttons.hasOwnProperty(tb)) {
				...
				switch(button.name) {
					...
					case 'insertImage':
						...
						toolbar += ' <div class="dropdown-menu dropdown-caret dropdown-menu-right">\
							 <div class="input-group">\
								<input class="form-control wys_yhb_insert_tu" placeholder="'+button.placeholder+'" type="text" data-edit="'+button.name+'" />\
								<span class="input-group-btn">\
									<button class="wys_yhb_insert_tu_btn btn btn-sm ' + button.button_insert_class + '" type="button">' + button.button_insert + '</button>\
								</span>\
							 </div>';
