...
public class NoReSetDEL_160622000081
{
    ...
    public string NRS_DEL(DataTable parameter_forUI)
    {
        ...
        if (ht_forUI.Contains("ajaxrun") && ht_forUI["ajaxrun"].ToString() == "del" && ht_forUI.Contains("oper") && ht_forUI["oper"].ToString() == "del" && ht_forUI.Contains("id") && ht_forUI["id"].ToString().Trim() != "")
        {
            ...
            for (int d = 0; d < delids.Length; d++)
            {
                ...
                alsql.Add("delete ZZZ_RZHT  where Hzhuangtai='草稿' and  HID=@HID_" + d);
            }
    ...
    /// <param name="parameter_forUI">前台表单传来的参数</param>
    /// <returns></returns>
    public string NRS_ZDY_shengxiao(DataTable parameter_forUI)
    {
        //接收转换参数
        Hashtable ht_forUI = new Hashtable();
        for (int i = 0; i < parameter_forUI.Rows.Count; i++)
        ...
        if (ht_forUI.Contains("zdyname") && ht_forUI["xuanzhongzhi"].ToString() != "")
        {
            ...
            for (int d = 0; d < ids.Length; d++)
            {
                if (ids[d].Trim() != "")
                {
                    ...
                    alsql.Add("UPDATE ZZZ_RZHT SET  Hzhuangtai='生效'  where HID =@HID_" + d);
                    alsql.Add("UPDATE ZZZ_fangzi SET  Fzhuangtai='已入住'  where FID = (select top 1 H_FID from ZZZ_RZHT where HID=@HID_" + d + ")");
                    alsql.Add("UPDATE ZZZ_chengzuren SET  Czhuangtai='已承租'  where CID =(select top 1 H_CID from ZZZ_RZHT where HID=@HID_" + d + ")");
                }
