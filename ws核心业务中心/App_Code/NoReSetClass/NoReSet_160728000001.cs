...
public class NoReSet_160728000001
{
    ...
    public DataSet NRS_EDIT(DataTable parameter_forUI)
    {
        ...
        param.Add("@Mbiaoti", ht_forUI["Mbiaoti"].ToString());
 
        param.Add("@Mdizhi", ht_forUI["Mdizhi"].ToString());
        param.Add("@Mbeiwanglu", ht_forUI["Mbeiwanglu"].ToString());
 
        alsql.Add("UPDATE ZZZ_MYGZT SET Mbiaoti=@Mbiaoti,Mdizhi=@Mdizhi,Mbeiwanglu=@Mbeiwanglu  where MID=@MID ");
    ...
    public DataSet NRS_EDIT_INFO(DataTable parameter_forUI)
    {
        ...
        return_ht = I_DBL.RunParam_SQL("select  top 1 *, Mdizhi as Mxuancaidan from ZZZ_MYGZT where MID=@MID", "数据记录", param);
